    ...
    def _sample_noise(self, x: torch.tensor, num: int, batch_size,sigma,noise_name) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            for _ in range(ceil(num / batch_size)):
                ...
                # mean_batch = mean.repeat((this_batch_size, 1, 1, 1)).reshape(batch)
                # variance_batch = variance.repeat((this_batch_size, 1, 1, 1)).reshape(batch)
                lambd = transform_lambda(noise_name, sigma)
                noise_input = noise_baselines(noise_name, batch, lambd=lambd)
                # noise = mean_batch + noise * variance_batch
